...
buildConfigGeneratorConfig: # Configuration for how the new build configs should be created
    ...
    reselectEnvironmentForExistingBuildConfigs: false # If true, reruns environment detection for managed, exact-match, and similar existing Build Configs. The existing JDK and build type are retained when SCM detection cannot determine them.
